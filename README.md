...
注意到 $t\le 3$，于是想到对 $t$ 的值做分类讨论。$t\leq 2$ 时若拿走了 $u$ 点的蝴蝶，但是没有立即前往 $u$ 点的某个儿子 $v$ 取走所有的蝴蝶，那么以后就必然无法再拿走 $v$ 点的蝴蝶了。而 $t=3$ 在 $t\le 2$ 的基础上增加了一种情况：拿走了 $u$ 点的蝴蝶，走到另一个距离 $u$ 点 $1$ 单位的点 $w$，然后回到 $u$，再拿走 $v$ 点的蝴蝶。
